...
| 32 | [DockSmith (StepFun)](https://zhangshuibai.github.io/paper-notes/agents/notes/32_DockSmith_2602.00592.html) | arXiv:2602.00592 v1 2026-01-31 → v2 2026-04-28 / StepFun · 专吃 Docker 环境构建的 agentic 模型 / base Qwen3-Coder-30B-A3B-Instruct + SFT only / 数据来自 <strong>SWE-Factory pipeline</strong>(4 agent: Context Retrieval / Dockerfile / Eval Script / Test Analysis)+ 两扩展(<strong>loop-detection controller</strong> 打破 agent 死循环 + <strong>cross-task success memory</strong> 跨任务复用已验证 Dockerfile)/ trajectory filter (acceptance shaping) + Score=0.5L+5R+3P complexity curriculum (Easy:Med:Hard=1:2:2) + joint training with Nex Agent-SFT (SWE:Docker 最佳 1:0.5 或 1:1) / <strong>Multi-Docker-Eval 开源 SOTA 39.72% F2P + 58.28% Commit Rate</strong>(base 19.46 → +20.26 pp · 超 DeepSeek-v3.1 37.72 / Kimi-K2-0905 37.62 / Claude-Sonnet-4 35.53 / GPT-5-Mini 34.13)/ 语言级 Python/JS/Java/Go/Ruby/Rust 全部第一,C 反退步 0%、C++/Java 仍低位 / 3 OOD 涨幅: SWE.V 49.65→51.90 (+2.25) · SWE.M 31.83→33.92 (+2.09) · TB2.0 10.67→14.04 (+3.37) / 错误分析: Total Errors −42.5%, Test Analysis stage −50.6%, Context Retrieval +10.9% (more aggressive recall) / Figure 4 within-layer persistence: P(env→env) +6%, P(runtime→runtime) +7% / Table 7 principled reasoning 27.1%→33.0% / 开源现状: 论文 abstract "<em>publicly available at here</em>" 占位符空 · HF 仅匿名账号 <code>8sj7df9k8m5x8/DockSmith</code>(model)+ <code>docker_building_training</code>(dataset Apache-2.0 39,719 samples / 2,876 instances)· 训练代码 / pipeline 实现 / loop-detection 算法 / cross-task memory retrieval 全未公开 / 局限: loop-detection + cross-task memory 两个"contribution"都<strong>无 ablation</strong> · OOD 涨幅小数量级,可能含源数据软污染 · 无 RL · MDE 协议 alignment 友好 / 链 #18 #20 #22 #23 #25 #29 + Nex-N1 (2512.04987) + SWE-Factory (2506.10954) + MDE (2512.06915) | 65 KB |
| 33 | [OpenEnv ← Agent World Model (Meta + Snowflake)](https://zhangshuibai.github.io/paper-notes/agents/notes/33_OpenEnv_AWM.html) | Meta PyTorch OpenEnv 把 Snowflake <strong>AgentWorldModel-1K (1,000 scenario × 10 task)</strong> 包装成标准 OpenEnv server / wire = WebSocket + FastAPI + JSON Action/Observation / <code>uvicorn ... envs.agent_world_model_env.server.app:app</code> 起 server / client <code>async with AWMEnv(base_url=...) as env</code> / Action 只有 <code>ListToolsAction()</code> 和 <code>CallToolAction(tool_name, arguments)</code> + 3 个保留 tool: <code>verify</code> / <code>done</code> / <code>__list_scenarios__</code> / <strong>verifier 两模式 = 同 tool 不同 args</strong>: <code>code</code> (默认 · SQLite 状态 diff + Python · 无 LLM 免费) / <code>sql</code> (SQL 提 diff + LLM-judge · <code>OPENENV_AWM_LLM_BASE_URL/API_KEY/MODEL</code> env var) / default reward = {complete:+1.0, incomplete:+0.1, format_error:−1.0, server_error:0.0} / reset 可覆盖 <code>reward_config</code> / done(keep_session=True) 留 5 文件 artifact: trajectory.json + {scenario}.db + {scenario}_initial.db + server.py + server.log / <strong>10,000 默认 MAX_CONCURRENT_ENVS</strong> · 3,000 idle 阈值 · 600s cleanup · 5s 扫描 / 1,024 episode × 64 concurrency × 3-20 turn stress test 脚本 / 公开 HF Space <code>ChilleD/agent_world_model_env</code> 零部署 / OpenEnv 自身 BSD-3 + AWM 数据 Apache-2.0 = 全栈商用 friendly / 跟 #18 原版差异: protocol WebSocket 化 + verify 内化为 tool (reward leak 风险) + subprocess 隔离 + idle cleanup + HF hosting / 跟 #31 verifiers 互桥 (OpenEnvAdapter) / 警告: scenario subprocess <strong>无 per-process sandbox</strong>,仅靠 Docker 兜底,官方"only academic research use" / 未确认 5 项 (pip path · Dockerfile · tool 总数 · default mode 翻转 · verify 调用次数限制) / 链 #18 #20 #22 #23 #29 #31 | 53 KB |
| 34 | [ToolMind (Nanbeige / BOSS Zhipin × RUC)](https://zhangshuibai.github.io/paper-notes/agents/notes/34_ToolMind_2511.15718.html) | arXiv:2511.15718 v1 2025-11-12 → v2 2025-12-05 / Nanbeige Lab BOSS 直聘 + 人大高瓴 Wayne Xin Zhao 组 / <strong>360K tool-agentic SFT</strong>(精确 368,611 = 163,180 graph-synth + 205,431 augmented open-source 跨 7 源: xLAM-60K 101K · Glaive-v2 20K · ToolACE 7.3K · BUTTON 21K · APIGen-MT 25K · When2Call 17K · τ-bench train 12.8K)/ <strong>方法 3 件套</strong>: (1) <strong>function graph + random walk</strong>(20K function · 参数级 embed · sᵢⱼ=max cos · τ 阈值+LM validator+随机扰动 · chain L~U{5..20} per-node visit limit) (2) <strong>三角色 multi-agent simulator</strong>(user incremental disclosure ###STOP### / assistant &lt;think&gt;+content+tool_calls / tool 模拟 server 含 error mode · <strong>tools NOT executable</strong>)(3) <strong>双层 binary filter</strong>: trajectory-level + <strong>turn-level mask 不丢</strong>(novelty)/ <strong>BFCL-v4</strong>: Qwen3-8B 42.21→46.92 (+4.69) · 14B 45.14→50.54 (+5.40) Multi-Turn +6.88 Agentic Search +23.00 但 Agentic Memory 14B 反退 −3.44 / <strong>τ-bench</strong>: 8B 35.83→46.70 (+10.87) · 14B 38.78→<strong>53.00</strong> (+14.22) retail 域 60.00 最强 / <strong>τ²-bench</strong>: 8B 34.64→46.41 (+11.77) · 14B 40.65→49.08 (+8.43) airline 14B +20.00 但 telecom 14B −1.75 / <strong>Table 4 消融</strong>: turn-filter 在 τ-bench +7.00 BFCL +2.76 <strong>但 τ²-bench −2.88(反例)</strong> / 训练: Qwen3-8B/14B + OpenRLHF + 64K seq + bs64 + lr 5e-6 + warmup 0.03 + loss only on anchored assistant / <strong>开源状态</strong>: HF dataset <code>Nanbeige/ToolMind</code> Apache-2.0 (2,759 下载 / 147 likes / 7.87GB / 8 jsonl split) 但<strong>含 ToolACE CC-BY-NC 派生</strong>实际是混 license · <strong>代码 / pipeline / judge LM 未公开</strong> · paper 主线 ckpt <strong>未发布</strong>(后续衍生 <code>ToolMind-Web-3B</code> 1,622 下载 转 deep-search Llama-base 不是 Qwen3)/ <strong>局限</strong>: turn-judge 用什么 LM 论文未说(可能 GPT-4o 但未确认) · binary 0/1 信号粗(改 weighted/3-class 应更好) · tool 是 LM 模拟非真 server(没在 MCP-Universe/MCPMark 真 server 评测) · NC 来源 license 混合 · "20K tools" 实为 schema 而非 callable / <strong>定位</strong>: TOUCAN 的方法论补丁版(turn-level mask 替代 trajectory-level Likert · graph-walk 替代 5-LLM 出题 · simulated tool 替代真 MCP) / 链 #18 #20 #22 #23 #28 #29 #30 #31 | 60 KB |
| 35 | [ToolACE (Huawei Noah's Ark + USTC)](https://zhangshuibai.github.io/paper-notes/agents/notes/35_ToolACE_2409.00920.html) | arXiv:2409.00920 v1 2024-09-02 → v2 2025-07-25 · <strong>ICLR 2025</strong> · 27 作者 Weiwen Liu/Xingshan Zeng/Yasheng Wang.../Enhong Chen / <strong>26,507 合成 API · 11,300 公开 dialog (HF subset, 完整训练集 ≥180K 未公开)</strong> / <strong>3 件套 TSS/SDG/DLV</strong>(<em>不是</em> TAS/SAD/DLA): (1) <strong>TSS Tool Self-Evolution Synthesis</strong> = speciation (pretraining 文档抽 API context tree) → adaptation (子树切 functionality) → evolution (4 类 diversity indicator 变异 + example buffer) (2) <strong>SDG Self-Guided Dialog Generation</strong> = <em>待训 LLM 当 complexity evaluator</em> (用 sample loss 当难度信号) + 3 agent (user/assistant/tool) multi-agent, 4 类 dialog (single/parallel/dependent/non-tool-use), tool 由 LM 模拟<strong>不接真 server</strong> (3) <strong>DLV Dual-Layer Validation</strong> = rule checker (4 维 API def/exec/dialog/consistency) + model checker (任务分解到 multiple expert agent) · ablation w.o.dual &lt; w.o.model &lt; Final / <strong>训练</strong> LLaMA-3.1-8B-Instruct full SFT → <strong>ToolACE-8B</strong> · <strong>BFCL-v3 Rank 3 / Overall 59.22 / Single AST 89.27 / Exec 90.07 / Rel 85.37 / Irrel 83.81 / Multi-turn 14.37 (弱点)</strong> · API-Bank "<em>comparable with GPT-4 series</em>" · MMLU/GSM8K/HumanEval/CSQA 通用能力无 degradation (vs xLAM-7b-fc-r 显著高) / <strong>🚨 LICENSE 校订verdict</strong>: <span style="color:#1f7a4a">HF API + README header 双验证 <code>Team-ACE/ToolACE</code> dataset + <code>ToolACE-8B</code>/<code>2-8B</code>/<code>2.5-8B</code> 三 model <strong>全 Apache-2.0</strong></span>(实地 curl 验证), <strong>不是</strong> CC-BY-NC-4.0 — 这意味着 #22 / #31 §10.1.1 / #34 中关于 "ToolACE 是 NC" 的论断<strong>错的</strong>, 需更正; ToolMind 含 7,327 条 ToolACE 派生<strong>没有 license 冲突</strong> / 真 NC 的同期数据是 xLAM-60K + Hammer-data, ToolACE 不是 / <strong>开源短板</strong>: pipeline 代码全闭(github 全 404 无 official repo, 只有数据 + 模型 weights) / <strong>团队衍生链</strong>: ToolACE-R (2504.01400 adaptive refinement) → ToolACE-DEV (2505.07512 decomposition+evolution) → ToolACE-MT (2508.12685 non-AR multi-turn) + 姐妹 paper ACEBench (#30) / <strong>community fork</strong>: watt-tool-8B (Apache, +DMPO multi-turn 偏好, 被 #29 EnvTuning 当 baseline 35.74→54.34) / 链 #22 #27 #28 #29 #30 #31 #34 | 56 KB |
| 36 | [📱 手机本地能跑的 Agent 小模型 Landscape](https://zhangshuibai.github.io/paper-notes/agents/notes/36_MobileAgents_landscape.html) | iOS+Android 端侧 / ≤4B base 模型 + mobile-agent 专训 + benchmark + 推理框架 全梳理 · 姊妹于 #16 桌面版 / **base 模型矩阵**(Apple Foundation ~3B 30 tok/s 官方 · Gemini Nano 3.25B via AICore · Qwen2.5-VL-3B Apache-2.0 phone-use · Phi-3.5-mini · Gemma 3 1B/4B · LLaMA 3.2 1B/3B · MiniCPM-V · MobileLLM 125M/350M · SmolLM2 1.7B IFEval 56.7 BFCL 27%)/ **mobile-agent 专训** (Octopus v2 2B functional-token 35× over Llama-7B · Mobile-Agent v1→v2→v3.5 阿里 X-PLUG / GUI-Owl 2B/4B/8B/32B Qwen3-VL base · DigiRL Stanford 1.3B AitW 17.7→**67.2** offline-to-online RL · UI-TARS-1.5 7B 跨平台)/ **benchmark** (AndroidWorld 116 task × 20 app paper 30.6% 现 leaderboard 70%+ · AitW 715K episode × 30K 指令 · AndroidControl 15.3K demo × 833 app NeurIPS 2024 · GUI-Odyssey · Mobile-Eval · B-MoCA · SPA-Bench)/ **推理栈** (MLC-LLM TVM Apache iOS+Android SDK · ExecuTorch Meta BSD 12+ 后端 CoreML/XNNPACK/Qualcomm/MediaTek 已上 IG/WA/Quest 3 · MediaPipe LLM Inference Google · Apple Core ML + ANE · MLX · llama.cpp + GGUF · MNN · NCNN)/ **硬件约束** (iPhone 15 Pro 8 GB 第三方 app ~3 GB hard / Increased Memory Limit entitlement ~5 GB / int4 必修 / NPU 内存带宽 bound) / **决策树 SVG** + 实测 tok/s 表 (除 Apple 30 之外多为社区数量级估计标 "~估计") / **诚实分析**: Siri/Google Assistant 仍走云 = 1-3B multi-turn 弱+VLM 太贵+iOS 跨 app 权限闭 / GUI agent 比 function-call 难一量级 (VLM 3-7B vs LLM 0.5-3B · 3+ GB RAM vs 1.5 / 每 step 1-3s vs &lt;1s) / 链 #14 #15 #16 #17 #27 #29 #31 | 54 KB |
| 37 | [AgentDistill (Princeton Mengdi Wang)](https://zhangshuibai.github.io/paper-notes/agents/notes/37_AgentDistill_2506.14728.html) | arXiv:2506.14728 v1 2025-06-17 / Princeton AI Lab + UMich + IIIS Tsinghua + SJTU + Columbia + CUHK + TCCI + UT Austin · Jiahao Qiu, Xinzhe Juan, Yimin Wang, Ling Yang... <strong>Mengdi Wang 通讯</strong> / <strong>"training-free agent distillation"</strong> 范式立项: ∇θπ_S = 0 student 权重<strong>完全冻结</strong>, 把"agent capability"从权重搬到运行时<strong>工具库</strong> / 同源于 Alita [20] / <strong>三阶段</strong>: (1) Teacher = Claude-Sonnet-4 (Manager) + GPT-4o (MCP creator, 4 个 sub-section Brainstorm→OpenSourceSearch→ScriptGen→VirtualExec) 解题时<em>顺手</em>生成 @mcp.tool() Python 脚本 → 仅留 π_T(x)=y 成功 traj + 可执行 MCP 得 pool L (2) <strong>MCP-Box construction</strong> 由 Claude-Sonnet-4 三步: <strong>Abstract</strong>(留 ≤3 critical param 公式 4)→ <strong>Cluster</strong>(按 functional semantic 聚 K cluster 公式 5)→ <strong>Consolidate</strong>(union param + if-branch 合并成 super-tool 公式 6)→ B={(MCP_k^final, cluster_name_k)} (3) <strong>SmolAgents framework</strong> 整 box mount 到 student tool interface <em>无 retrieval 无 reranking 无 param selection</em> · MCP 实为 <strong>FastMCP 同进程 Python 函数</strong>(不接真 cross-process server) / <strong>实测 K 极小</strong>: PathVQA 9 · SLAKE 13 · Game-of-24 仅 <strong>1</strong> / Table 1 calling rate: Qwen3-8B SLAKE 94.7% · GPT-3.5 PathVQA 38.0% · LLaMA-3.1-8B PathVQA 24.3% / Table 2 <strong>9/9 全正向</strong>: GPT-3.5 Game24 34.3→82.7 (<strong>+48.4</strong>)· LLaMA-3.1-8B Game24 21.7→64.0 (<strong>+42.3</strong>) · LLaMA SLAKE 49.3→59.3 (+10.0) · Qwen3-8B Game24 ceiling +7 / Table 3 跨 agent: student-after-distill 在 PathVQA <strong>52.7 险胜 teacher 52</strong> 但 Game24 teacher 99 vs student 75.5 (solver 在但 student 仍读错数字)· OctoTools(GPT-4o) 49/64/45 被均压 / Figure 5 brain-MRI case study: 两 narrow MCP(亮区 / 左半边)consolidate 成 region+analysis_mode+3 threshold 的 super-tool / <strong>开源</strong>: <strong>无</strong> · GitHub <code>shimu0215/AgentDistill</code> 是 <strong>另一篇 2505.17612 (Nardien et al.)</strong> 撞名 · arXiv "Code, Data and Media" tab 空 · HF 0 关联 / <strong>诚实批判</strong>: (a) "training-free" 是 student-side 0 grad 但 teacher-side Claude-4+GPT-4o token cost 未报, 数量级 $100-500/dataset (b) box 整挂无 retrieval 仅在 K≤13 成立, 不可 scale 到 BFCL 全 server (c) Game24 +48 实测 program-execution offload 而非 capability transfer, 确定性 puzzle 写 solver 就到顶 (d) 100 example box-gen 与 eval set 的 overlap 未明示, 可能 train=test (e) 跨 domain transfer 完全没测 — 每 box per-benchmark 训, "generalizable" 标题口号大于证据 (f) 缺 vs TOUCAN-SFT-8B / vs prompt-only baseline 的 head-to-head / <strong>定位</strong>: TOUCAN/ToolMind/ToolACE 是<em>权重派</em>(SFT 蒸馏), AgentDistill 是<em>库派</em>(工具运行时蒸馏), EnvTuning 是 env 派 — agent capability 应该放在哪是这一波 MCP 浪潮的根本二选一 / 链 #22 #27 #29 #31 #34 #35 + Alita | 46 KB |
...
| DockSmith: Scaling Reliable Coding Environments via an Agentic Docker Builder | StepFun (Jiaran Zhang, Luck Ma, Fanqi Wan, Di Qi, ..., Qi Han, Xiangyu Zhang) | 2026-01-31 (v1) · 2026-04-28 (v2) | [2602.00592](https://arxiv.org/abs/2602.00592) · [HF Model (anonymous)](https://huggingface.co/8sj7df9k8m5x8/DockSmith) · [HF Dataset (Apache-2.0)](https://huggingface.co/datasets/8sj7df9k8m5x8/docker_building_training) |
| ToolMind Technical Report: A Large-Scale, Reasoning-Enhanced Tool-Use Dataset | Nanbeige Lab, BOSS Zhipin + Gaoling School of AI, Renmin University of China (Chen Yang, Ran Le, ..., Wayne Xin Zhao, Yang Song, Tao Zhang) | 2025-11-12 (v1) · 2025-12-05 (v2) | [2511.15718](https://arxiv.org/abs/2511.15718) · [HF Dataset (Apache-2.0)](https://huggingface.co/datasets/Nanbeige/ToolMind) · [HF Org Nanbeige](https://huggingface.co/Nanbeige) |
| ToolACE: Winning the Points of LLM Function Calling | Huawei Noah's Ark Lab + USTC + CUHK (Weiwen Liu, Xu Huang, Xingshan Zeng, ..., Yasheng Wang, Ruiming Tang, Defu Lian, Qun Liu, Enhong Chen, 27 作者) | 2024-09-02 (v1) · 2025-07-25 (v2) · **ICLR 2025** | [2409.00920](https://arxiv.org/abs/2409.00920) · [HF Org Team-ACE](https://huggingface.co/Team-ACE) · [HF Dataset (Apache-2.0)](https://huggingface.co/datasets/Team-ACE/ToolACE) · [HF Model ToolACE-8B (Apache-2.0)](https://huggingface.co/Team-ACE/ToolACE-8B) · [OpenReview](https://openreview.net/forum?id=8EB8k6DdCU) |
| AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes | Princeton AI Lab + UMich + IIIS Tsinghua + SJTU + Columbia + CUHK + TCCI + UT Austin (Jiahao Qiu, Xinzhe Juan, Yimin Wang, Ling Yang, Xuan Qi, Tongcheng Zhang, Jiacheng Guo, Yifu Lu, Zixin Yao, Hongru Wang, Shilong Liu, Xun Jiang, Liu Leqi, **Mengdi Wang** 通讯) | 2025-06-17 (v1) | [2506.14728](https://arxiv.org/abs/2506.14728) · *no official code/data/model released as of 2026-05* |
